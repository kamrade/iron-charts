...
// NORMALIZE DATA
	...
	var drawLine = function(key, data){
		...
		var dots = svg.append("g")
			...
			.enter()
				...
				// EVENTS
				.on("click", function(e){
					if(svg[0][0].lastChild != this.parentNode){
						var className = "path-" +  (this.parentNode.className.animVal).substr(5);
						var par = this.parentNode.parentNode;
						var line = par.querySelector("g." + className);
						par.removeChild(line);
						svg[0][0].appendChild(line);
						var group = this.parentNode;
						group.parentNode.removeChild(group);
						svg[0][0].appendChild(group);
					}
				})

				.on("mouseenter", function(d){
					d3.select(this).attr("r", nodeRadius*2);
					tooltip.style.left = d3.event.target.cx.animVal.value - tooltip.clientWidth/2 + "px";
					tooltip.style.top = d3.event.target.cy.animVal.value - tooltip.clientHeight*1.5 + "px";
					tooltip.innerText = typeOfTransaction + " | " + key + "\n" + formatDate(d.date) + " : " + d[ values[typeOfTransaction][key]  ];
		...
		var reload = function(filename){
			d3.json(filename, function(rows){
				if(rows){
					...
					redraw(rows);
					// dataMain инициализируется в конце далее в модуле при реализации ф-ции resize...
					// если dataMain == undefined, то модуль еще не прорисовался и ресайзить пока нечего
					dataMain = rows;
	...
	var redraw = function(data) {
		...
		barsFat
			...
			// EVENTS
			.on("click", function(e){
				var child4 = svg[0][0].children[4];
				var child5 = svg[0][0].children[5];
				child4.parentNode.removeChild(child4);
				child5.parentNode.removeChild(child5);
				svg[0][0].appendChild(child4);
				svg[0][0].appendChild(child5);
			});
